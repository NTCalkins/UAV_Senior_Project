...
#define PER_THRESH .0005		//Threshold for the percentage of pixels in a section that are below the DIS_THRESH
...
int main()
{
    ...
    init_params.depth_mode = DEPTH_MODE_QUALITY;	//Can be set to PERFORMANCE, MEDIUM, OR QUALITY
    init_params.coordinate_units = sl::UNIT_METER;	//Measurements are in meters
	...
	//Create a Python Interpreter
   	...
   	PyObject *pName, *pMod, *pDict;
    	...
    	pName = PyString_FromString("PyMovement");
	//cout << "Called Python Functions\n";
...
//cin.ignore();
	...
	while (key != 'x')	//Used to exit the program
	{
		...
		while (key != 'q')
		{
			...
			//{
				...
				// Resize and display with OpenCV
					...
					int section = 144;//selectSection(sectionValues);		//The section that is selected
		...
		//			key = cv::waitKey(1);
					...
					string picName = "images/depthImage" + to_string(counter) + ".jpg";
					//string leftName = "images/Left" + to_string(counter) + ".jpg";
		    			//string rightName = "images/Right" + to_string(counter) + ".jpg";
					cv::imwrite(picName, depth_image_ocv_display);
					//cv::imwrite(leftName, leftImage_display);
		    			//cv::imwrite(rightName, rightImage_display);
...
void calcPercentages(float *sectionValues, const int *sections)
{
	...
	//cout << "Section " << 144 << ": " << sections[144] << " / " << TOTAL_PIXELS << endl;
	//cout << "Section " << 144 << ": " << sectionValues[144] << "%\n";
	//file.close();
}
...
{
	PyObject *pFunc, *pArgs, *pValRight, *pValUp;    //PyObjects to call the Python script
	...
	{
		double delta_w = centerW - CENTER_WIDTH;   //Get the difference between the center of the image and the center of the section (W)
		cout << "DW: " << delta_w;
		double delta_h = centerH - CENTER_HEIGHT;  //Get the difference between teh center of the image and the center of the section (H)
		cout << "DH: " << delta_h;
		double theta;
		if(delta_w == 0)
			theta = PI / 2;
		else
			theta = atan(abs(delta_h) / abs(delta_w));
	...
	{
		double DISTANCE_RIGHT = DISTANCE * cos(theta);  //Get the distance to the right
		if(delta_w < 0)
			DISTANCE_RIGHT *= -1;
		double DISTANCE_UP = DISTANCE * sin(theta);  //Get the distance up
		if(delta_h > 0)
			DISTANCE_UP *= -1;	
	cout << "Distance Right: " << DISTANCE_RIGHT;
	cout << "Distance Up: " << DISTANCE_UP;
	    ...
	    pValRight = PyFloat_FromDouble(DISTANCE_RIGHT); //Set the value of pValRight to the right distance
	    pValUp = PyFloat_FromDouble(DISTANCE_UP); //Set the value of pValUp to the up distance
	    PyTuple_SetItem(pArgs, 0, pValRight);   //Set the first parameter to the y distance
	    PyTuple_SetItem(pArgs, 1, pValUp);   //Set the second parameter to the z distance

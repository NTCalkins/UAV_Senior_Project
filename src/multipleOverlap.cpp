...
int main()
{
    ...
    pName = PyString_FromString("DroneTestTakeoff");
...
void manuever(const int& section, const int& centerW, const int& centerH, PyObject *pDict)
{
	...
	else
	{
		...
		{
			double VELO_Y = VELO * cos(theta);  //Get the Y velocity
			double VELO_Z = VELO * sin(theta);  //Get the Z velocity
		    ...
		    pValY = PyFloat_FromDouble(VELO_Y); //Set the value of pValY to the y velo
		    pValZ = PyFloat_FromDouble(VELO_Z); //Set the value of pValZ to the z velo
		    PyTuple_SetItem(pArgs, 0, pValY);   //Set the first parameter to the y velo
		    PyTuple_SetItem(pArgs, 1, pValZ);   //Set the second parameter to the z velo
